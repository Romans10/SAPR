...
DataValidationPrivate::DataValidationPrivate()
    : validationType(DataValidation::Type::None)
    , validationOperator(DataValidation::Operator::Between)
    , errorStyle(DataValidation::ErrorStyle::Stop)
    , allowBlank(false)
...
DataValidationPrivate::DataValidationPrivate(
    DataValidation::Type type, DataValidation::Operator op, const QString &formula1, const QString &formula2, bool allowBlank)
    : validationType(type)
 ...
 */
DataValidation::Type DataValidation::validationType() const
{
 ...
 */
void DataValidation::setType(DataValidation::Type type)
{
 ...
 */
void DataValidation::setOperator(DataValidation::Operator op)
{
...
{
	static const QMap<DataValidation::Type, QString> typeMap = {{DataValidation::Type::None, QStringLiteral("none")},
	                                                            {DataValidation::Type::Whole, QStringLiteral("whole")},
	                                                            {DataValidation::Type::Decimal, QStringLiteral("decimal")},
	                                                            {DataValidation::Type::List, QStringLiteral("list")},
	                                                            {DataValidation::Type::Date, QStringLiteral("date")},
	                                                            {DataValidation::Type::Time, QStringLiteral("time")},
	                                                            {DataValidation::Type::TextLength, QStringLiteral("textLength")},
	                                                            {DataValidation::Type::Custom, QStringLiteral("custom")}};

	static const QMap<DataValidation::Operator, QString> opMap = {{DataValidation::Operator::Between, QStringLiteral("between")},
	                                                              {DataValidation::Operator::NotBetween, QStringLiteral("notBetween")},
	                                                              {DataValidation::Operator::Equal, QStringLiteral("equal")},
	                                                              {DataValidation::Operator::NotEqual, QStringLiteral("notEqual")},
	                                                              {DataValidation::Operator::LessThan, QStringLiteral("lessThan")},
	                                                              {DataValidation::Operator::LessThanOrEqual, QStringLiteral("lessThanOrEqual")},
	                                                              {DataValidation::Operator::GreaterThan, QStringLiteral("greaterThan")},
	                                                              {DataValidation::Operator::GreaterThanOrEqual, QStringLiteral("greaterThanOrEqual")}};

	static const QMap<DataValidation::ErrorStyle, QString> esMap = {{DataValidation::ErrorStyle::Stop, QStringLiteral("stop")},
	                                                                {DataValidation::ErrorStyle::Warning, QStringLiteral("warning")},
	                                                                {DataValidation::ErrorStyle::Information, QStringLiteral("information")}};

	writer.writeStartElement(QStringLiteral("dataValidation"));
	if (validationType() != DataValidation::Type::None)
		writer.writeAttribute(QStringLiteral("type"), typeMap[validationType()]);
	if (errorStyle() != DataValidation::ErrorStyle::Stop)
		writer.writeAttribute(QStringLiteral("errorStyle"), esMap[errorStyle()]);
	if (validationOperator() != DataValidation::Operator::Between)
...
DataValidation DataValidation::loadFromXml(QXmlStreamReader &reader)
{
	...
	static const QMap<QString, DataValidation::Type> typeMap = {{QStringLiteral("none"), DataValidation::Type::None},
	                                                            {QStringLiteral("whole"), DataValidation::Type::Whole},
	                                                            {QStringLiteral("decimal"), DataValidation::Type::Decimal},
	                                                            {QStringLiteral("list"), DataValidation::Type::List},
	                                                            {QStringLiteral("date"), DataValidation::Type::Date},
	                                                            {QStringLiteral("time"), DataValidation::Type::Time},
	                                                            {QStringLiteral("textLength"), DataValidation::Type::TextLength},
	                                                            {QStringLiteral("custom"), DataValidation::Type::Custom}};

	static const QMap<QString, DataValidation::Operator> opMap = {{QStringLiteral("between"), DataValidation::Operator::Between},
	                                                              {QStringLiteral("notBetween"), DataValidation::Operator::NotBetween},
	                                                              {QStringLiteral("equal"), DataValidation::Operator::Equal},
	                                                              {QStringLiteral("notEqual"), DataValidation::Operator::NotEqual},
	                                                              {QStringLiteral("lessThan"), DataValidation::Operator::LessThan},
	                                                              {QStringLiteral("lessThanOrEqual"), DataValidation::Operator::LessThanOrEqual},
	                                                              {QStringLiteral("greaterThan"), DataValidation::Operator::GreaterThan},
	                                                              {QStringLiteral("greaterThanOrEqual"), DataValidation::Operator::GreaterThanOrEqual}};

	static const QMap<QString, DataValidation::ErrorStyle> esMap = {{QStringLiteral("stop"), DataValidation::ErrorStyle::Stop},
	                                                                {QStringLiteral("warning"), DataValidation::ErrorStyle::Warning},
	                                                                {QStringLiteral("information"), DataValidation::ErrorStyle::Information}};
	...
	if (attrs.hasAttribute(QLatin1String("type"))) {
		...
		validation.setType(it != typeMap.constEnd() ? it.value() : DataValidation::Type::None);
	}
	if (attrs.hasAttribute(QLatin1String("errorStyle"))) {
		...
		validation.setErrorStyle(it != esMap.constEnd() ? it.value() : DataValidation::ErrorStyle::Stop);
	}
	if (attrs.hasAttribute(QLatin1String("operator"))) {
		...
		validation.setOperator(it != opMap.constEnd() ? it.value() : DataValidation::Operator::Between);
	}
