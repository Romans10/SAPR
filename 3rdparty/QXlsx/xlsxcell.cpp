...
/*!
  \enum Cell::Type
  \value Boolean      Boolean type
  \value Number       Number type, can be blank or used with forumula
  \value Error        Error type
  \value SharedString Shared string type
  \value String       String type, can be used with forumula
  \value InlineString Inline string type
  */
 ...
 */
// qint32 styleIndex = (-1)
Cell::Cell(const QVariant &data, Type type, const Format &format, Worksheet *parent, qint32 styleIndex)
...
bool Cell::isRichString() const
{
	...
	if (d->cellType != Type::SharedString && d->cellType != Type::InlineString && d->cellType != Type::String) {
		return false;
	}
