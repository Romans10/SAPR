...
bool Worksheet::write(int row, int column, const QVariant &value, const Format &format)
{
	...
	} else if (value.userType() == QMetaType::Int || value.userType() == QMetaType::UInt || value.userType() == QMetaType::LongLong
...
bool Worksheet::writeString(int row, int column, const RichString &value, const Format &format)
{
	...
	QSharedPointer<Cell> cell = QSharedPointer<Cell>(new Cell(value.toPlainString(), Cell::Type::SharedString, fmt, this));
...
bool Worksheet::writeInlineString(int row, int column, const QString &value, const Format &format)
{
	...
	Format fmt = format.isValid() ? format : d->cellFormat(row, column);
	d->workbook->styles()->addXfFormat(fmt);
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(value, Cell::Type::InlineString, fmt, this));
	return true;
}
...
bool Worksheet::writeNumeric(int row, int column, double value, const Format &format)
{
	...
	Format fmt = format.isValid() ? format : d->cellFormat(row, column);
	d->workbook->styles()->addXfFormat(fmt);
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(value, Cell::Type::Number, fmt, this));
	return true;
}
...
bool Worksheet::writeFormula(int row, int column, const CellFormula &formula_, const Format &format, double result)
{
	...
	QSharedPointer<Cell> data = QSharedPointer<Cell>(new Cell(result, Cell::Type::Number, fmt, this));
			...
			for (int c = range.firstColumn(); c <= range.lastColumn(); ++c) {
				if (!(r == row && c == column)) {
					if (Cell *cell = cellAt(r, c)) {
						...
						QSharedPointer<Cell> newCell = QSharedPointer<Cell>(new Cell(result, Cell::Type::Number, fmt, this));
	...
	} else if (formula.formulaType() == CellFormula::Type::Shared) {
	}
...
bool Worksheet::writeBlank(int row, int column, const Format &format)
{
	...
	// Note: Number with an invalid QVariant value means blank.
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(QVariant(), Cell::Type::Number, fmt, this));
...
bool Worksheet::writeBool(int row, int column, bool value, const Format &format)
{
	...
	Format fmt = format.isValid() ? format : d->cellFormat(row, column);
	d->workbook->styles()->addXfFormat(fmt);
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(value, Cell::Type::Boolean, fmt, this));
...
bool Worksheet::writeTime(int row, int column, const QTime &t, const Format &format)
{
	...
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(timeToNumber(t), Cell::Type::Number, fmt, this));
...
bool Worksheet::writeHyperlink(int row, int column, const QUrl &url, const Format &format, const QString &display, const QString &tip)
{
	...
	*/
	...
	if (!fmt.isValid()) {
		fmt.setVerticalAlignment(Format::VerticalAlignment::Center);
		fmt.setFontColor(Qt::blue);
		fmt.setFontUnderline(Format::FontUnderline::Single);
	}
	d->workbook->styles()->addXfFormat(fmt);

	// Write the hyperlink string as normal string.
	d->sharedStrings()->addSharedString(displayString);
	d->cellTable[row][column] = QSharedPointer<Cell>(new Cell(displayString, Cell::Type::SharedString, fmt, this));
...
{
	Q_D(Worksheet);
	if (validation.ranges().isEmpty() || validation.validationType() == DataValidation::Type::None)
		return false;
...
int Worksheet::insertImage(int row, int column, const QImage &image)
{
	...
	DrawingOneCellAnchor *anchor = new DrawingOneCellAnchor(d->drawing.data(), DrawingAnchor::ObjectType::Picture);
...
QList<CellRange> Worksheet::mergedCells() const
{
	...
	if (d->type == AbstractSheet::Type::Work) {
		return d->merges;
	} else if (d->type == AbstractSheet::Type::Chart) {
	} else if (d->type == AbstractSheet::Type::Dialog) {
	} else if (d->type == AbstractSheet::Type::Macro) {
	} else {  // undefined
	}
...
void Worksheet::saveToXmlFile(QIODevice *device) const
{
	...
	// pageMargins
	if (false == d->PMleft.isEmpty() && false == d->PMright.isEmpty() && false == d->PMtop.isEmpty() && false == d->PMbotton.isEmpty()
...
void WorksheetPrivate::saveXmlCellData(QXmlStreamWriter &writer, int row, int col, QSharedPointer<Cell> cell) const
{
	...
	if (cell->type() == Cell::Type::SharedString)  // 's'
	{
		int sst_idx;
	...
	} else if (cell->type() == Cell::Type::InlineString)  // 'inlineStr'
	{
	...
	} else if (cell->type() == Cell::Type::Number)  // 'n'
	{
	...
	} else if (cell->type() == Cell::Type::String)  // 'str'
	{
	...
	} else if (cell->type() == Cell::Type::Boolean)  // 'b'
	{
	...
	} else if (cell->type() == Cell::Type::Date)  // 'd'
	{
		// dev67
	...
	} else if (cell->type() == Cell::Type::Error)  // 'e'
	{
		writer.writeAttribute(QStringLiteral("t"), QStringLiteral("e"));
		writer.writeTextElement(QStringLiteral("v"), cell->value().toString());
	} else  // if (cell->type() == Cell::Custom)
	{
		// custom type
	...
	while (!reader.atEnd() && !(reader.name() == QLatin1String("sheetData") && reader.tokenType() == QXmlStreamReader::EndElement)) {
		if (reader.readNextStartElement()) {
			if (reader.name() == QLatin1String("row")) {
				...
				if (attributes.hasAttribute(QLatin1String("customFormat")) || attributes.hasAttribute(QLatin1String("customHeight"))
				    || attributes.hasAttribute(QLatin1String("hidden")) || attributes.hasAttribute(QLatin1String("outlineLevel"))
			...
			} else if (reader.name() == QLatin1String("c"))  // Cell
			{
				...
				// Cell::Type cellType = Cell::Number;
				Cell::Type cellType = Cell::Type::Custom;
				...
				while (!reader.atEnd() && !(reader.name() == QLatin1String("c") && reader.tokenType() == QXmlStreamReader::EndElement)) {
					if (reader.readNextStartElement()) {
						if (reader.name() == QLatin1String("f"))  // formula
						{
							...
							if (formula.formulaType() == CellFormula::Type::Shared && !formula.formulaText().isEmpty()) {
...
QVector<CellLocation> Worksheet::getCells()
{
	Q_D(const Worksheet);
	...
	while (_it.hasNext()) {
		...
		int row = _it.key();
		QMapIterator<int, QSharedPointer<Cell>> _iit(_it.value());

		while (_iit.hasNext()) {
			_iit.next();

			int col = _iit.key();
			QSharedPointer<Cell> ptrCell = _iit.value();
